...
const MembershipManagement: React.FC = () => {
  ...
  const subscribedMembers = data.filter(member => member.subscribed);
  const unsubscribedMembers = data.filter(member => !member.subscribed);
    ...
    form.setFieldsValue(
      record
        ...
        : {
            name: '',
            email: '',
            status: activeTab === 'subscribed' ? 'active' : 'inactive',
            package: undefined,
            joinDate: dayjs(),
            subscribed: activeTab === 'subscribed',
          }
    );
    setVisible(true);
  };
      ...
      render: (_: any, record: any) => (
        <Space size="middle">
          ...
          <Button
            ...
            size="small"
            title={record.status === 'blocked' ? 'Unblock Member' : 'Block Member'}
          />
          <Button
            danger
            icon={<DeleteOutlined />}
  ...
  return (
    <div className="p-4">
      <div className="flex justify-between items-center mb-6">
        <Title level={2}>Membership Management</Title>
      ...
      <Card>
        <Tabs
          ...
          className="mb-4"
        >
          <TabPane tab="Subscribed Members" key="subscribed">
            <Table
              columns={columns}
          ...
          <TabPane tab="Unsubscribed Members" key="unsubscribed">
            <Table
              columns={columns}
              dataSource={unsubscribedMembers}
              rowKey="id"
              pagination={{ pageSize: 10 }}
            />
          </TabPane>
      ...
      >
        <Form form={form} layout="vertical" name="member_form">
          ...
          <Form.Item
            ...
            rules={[
              {
                required: form.getFieldValue('subscribed'),
          ...
          <Form.Item name="subscribed" label="Subscription Status">
            <Select
              onChange={value => {
                // Update the status field based on subscription status
                const currentStatus = form.getFieldValue('status');
                if (value && currentStatus === 'inactive') {
                  form.setFieldsValue({ status: 'active' });
                } else if (!value && currentStatus === 'active') {
